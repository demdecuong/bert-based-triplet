...
if __name__ == '__main__':
    ...
    # model.load_state_dict(torch.load('./ckpt/best_model_state_v1_triplet.bin'))
    model.eval()
    ...
    embed = build_database_embed(model,tokenizer,db,config)
    test_acc_top5 = compute_topk(model,tokenizer,test_src,test_idx,embed.to(device),config,top_k=5)
    test_acc_top3 = compute_topk(model,tokenizer,test_src,test_idx,embed.to(device),config,top_k=3)
    test_acc_top1 = compute_topk(model,tokenizer,test_src,test_idx,embed.to(device),config,top_k=1)
